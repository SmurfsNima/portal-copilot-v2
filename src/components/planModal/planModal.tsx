...
const PlanManagerModal: React.FC<PlanManagerModalProps> = ({ priorities }) => {
  ...
  const handleCheckboxChange = (
    ...
    if (subMenuIndex !== undefined) {
      newItems[categoryIndex].subCategories[subCategoryIndex].subMenus![
        subMenuIndex
      ].isActive =
        !newItems[categoryIndex].subCategories[subCategoryIndex].subMenus![
          subMenuIndex
  ...
  return (
    <div className="bg-black-secondary text-primary-text p-4 rounded-md border border-main-border shadow-lg w-full h-[75%]">
      <div className="w-full flex justify-between gap-3">
        {items.map((priority, index) => (
          <div
            key={priority.category}
            className="p-4 rounded-md bg-black-primary select-none w-[365px]"
            ...
            <div className="flex px-3 pb-1 justify-between items-center border-b border-main-border w-full">
              <span className="flex items-center gap-2 text-xs font-medium">
                <div className="bg-black-background rounded-lg p-1 flex items-center justify-center">
                  <div
                    className={`${theme}-icons-${priority.category} w-4 h-4 bg-brand-secondary-color`}
                  ></div>
                </div>
                {priority.category}
              </span>
                    ...
                    <label className="flex gap-1 items-center justify-start cursor-pointer text-xs font-normal text-secondary-text">
                      {subCategory.subMenus && (
                        <img
                          ...
                          className={`${
                            expanded[`${priority.category}-${subIndex}`]
                              ? "rotate-180"
                              : "rotate-90"
